...
!******************************************************************************
	...
	dx,dx2,dxx,dy,dy2,dyy,dt,Kappa2,KappaA,Rc,Cx,Cy,d6, &
	alpha, Bcoeff, CCoeff,Tol,OmTol,PsiTol, &
...
!***************************************************
	...
	call OmegaCalc(Nx,My,alpha,BCoeff,BCoeff,Omega, &
		Omega0,u,v,DM,DM2)
...
!
!**********************************************************************
	subroutine OmegaCalc(Nx,My,alpha,BCoeff,CCoeff,Omega, &
		Omega0,u,v,DM,DM2)
	integer Nx,My,i,j
	double precision Omega(Nx+2,My+2),U(Nx+2,My+2),v(Nx+2,My+2), &
		Omega0(Nx+2,My+2),alpha, Bcoeff, CCoeff, &
...
!$OMP DO
	...
	if ((i.gt.2).and.(i.lt.(Nx+1))) then
		Omega(i,j) = Omega(i,j) + -Cx/DM2(i,j) * (Omega0(i-2,j) - 8*Omega0(i-1,j) + 8*Omega0(i+1,j) - Omega0(i+2,j)) / 12
	else if (i.eq.2) then
		Omega(i,j) = Omega(i,j) -Cx/DM2(i,j) * (-1/4*Omega0(i-1,j) - 5/6*Omega0(i,j) + 3/2*Omega0(i+1,j) - &
		1/2*Omega0(i+2,j) + 1/12*Omega0(i+3,j)) / 12
	...
	if ((j.gt.2).and.(j.lt.(My+1))) then
		Omega(i,j) = Omega(i,j) -Cy/DM2(i,j) * (Omega0(i,j-2) - 8*Omega0(i,j-1) + 8*Omega0(i,j+1) - Omega0(i,j+2)) / 12
	else if (j.eq.2) then
		Omega(i,j) = Omega(i,j) -Cx/DM2(i,j) * (-1/4*Omega0(i,j-1) - 5/6*Omega0(i,j) + 3/2*Omega0(i,j+1) - &
		1/2*Omega0(i,j+2) + 1/12*Omega0(i,j+3)) / 12
...
!	Psi1m Maximum single value of the previous Psi matrix
...
!************************************************************************
	...
	double precision Omega(Nx+2,My+2),Psi(Nx+2,My+2),Psi0(Nx+2,My+2), &
		Kappa2,KappaA,dxx,Tol,PsiTol,Psi1m,x(Nx+2),y(My+2), &
		DM2(Nx+2,My+2)
